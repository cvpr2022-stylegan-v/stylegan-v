...
def setup_snapshot_image_grid(training_set, random_seed=0):
    ...
    l = [b['video_len'] for b in batches]

    return (gw, gh), np.stack(images), np.stack(labels), np.stack(t), np.stack(l)
...
def training_loop(
    ...
    if rank == 0:
        if not resume_whole_state:
            ...
            vis.grid_l = torch.from_numpy(vis.video_lens).to(device).split(batch_gpu) # (num_batches, [batch_size])
            ...
            vis.vgrid_l = torch.from_numpy(vis.video_lens)[:vis.num_videos].to(device) # [batch_size]
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            ...
            all_gen_l = torch.from_numpy(np.stack(all_gen_l)).pin_memory().to(device)
            all_gen_l = [phase_gen_t.split(batch_gpu) for phase_gen_t in all_gen_l.split(batch_size)]

        # Execute training phases.
        for phase, phase_gen_z, phase_gen_c, phase_gen_t, phase_gen_l in zip(phases, all_gen_z, all_gen_c, all_gen_t, all_gen_l):
            ...
            curr_data = zip(phase_real_img, phase_real_c, phase_real_t, phase_gen_z, phase_gen_c, phase_gen_t, phase_gen_l)
            for round_idx, (real_img, real_c, real_t, gen_z, gen_c, gen_t, gen_l) in enumerate(curr_data):
            ...
            if not G_ema.synthesis.motion_encoder is None:
                with torch.no_grad():
                    motion_noise = G_ema.synthesis.motion_encoder(
                        ...
                        t=vis.ts[[0]],
                        l=vis.vgrid_l[[0]],
                        w=torch.zeros(1, G_ema.w_dim, device=device))['motion_noise'] # [1, *motion_dims]
