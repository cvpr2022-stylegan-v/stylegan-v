...
NUM_FRAMES_IN_BATCH = {128: 256, 256: 256, 512: 128, 1024: 128}

#----------------------------------------------------------------------------

def compute_fvd(opts, max_real: int, num_gen: int, num_frames: int, subsample_factor: int=1):
    # Direct TorchScript translation of the TensorFlow I3D model, trained on Kinetics-400
    ...
    mu_real, sigma_real = metric_utils.compute_feature_stats_for_dataset(
        ...
        capture_mean_cov=True, max_items=max_real, temporal_detector=True, batch_size=batch_size // num_frames).get_mean_cov()
    ...
    mu_gen, sigma_gen = compute_gen_stats_fn(
        ...
        max_items=num_gen, temporal_detector=True, batch_size=batch_size // num_frames, **gen_kwargs).get_mean_cov()
