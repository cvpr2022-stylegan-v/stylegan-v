...
def project(
    ...
    l = torch.zeros(num_videos, device=device)
    ...
    if use_motion_init:
        motion_noise_opt = select_motions(motion_codes)
    else:
        motion_noise_opt = G.synthesis.motion_encoder(c=c, t=ts, l=l, w=torch.zeros(num_videos, G.w_dim, device=device))['motion_noise']
        # motion_noise_opt.data = torch.randn_like(motion_noise_opt.data) * 1e-3

    motion_noise_opt.requires_grad_(True)

    w_result = torch.zeros([num_steps] + list(w_opt.shape), dtype=torch.float32, device=device)
    # optimizer = torch.optim.Adam([w_opt] + [motion_noise_opt], betas=(0.9, 0.999), lr=initial_learning_rate)
    ...
    for step in tqdm(range(num_steps)):
        ...
        #synth_images = G.synthesis(ws, c=c, t=ts, l=l, motion_noise=motion_noise_opt + torch.randn_like(motion_noise_opt) * w_noise_scale)
        synth_images = G.synthesis(ws, c=c, t=ts, l=l, motion_noise=motion_noise_opt)
        #synth_images = G.synthesis(ws, c=c, t=ts, l=l)
        ...
        if motion_reg_type is None:
            ...
            reg_loss = motion_noise_opt.norm(dim=2).mean()
        elif motion_reg_type == "dist":
            reg_loss = motion_noise_opt.mean().pow(2) + (motion_noise_opt.var() - 1).pow(2)
        ...
        # Normalize noise.
    #     with torch.no_grad():
    #         for buf in motion_noise_opt.values():
    #             buf -= buf.mean()
    #             buf *= buf.square().mean().rsqrt()

    return w_result, motion_noise_opt

#----------------------------------------------------------------------------

@torch.no_grad()
def find_motions_init(G: "Generator", vgg16: nn.Module, target_features: Tensor, c: Tensor, t: Tensor, l: Tensor, num_motions_to_try: int=128):
    motions = G.synthesis.motion_encoder(
        c=c.repeat_interleave(num_motions_to_try, dim=0),
        t=ts.repeat_interleave(num_motions_to_try, dim=0),
        l=l.repeat_interleave(num_motions_to_try, dim=0),
        w=torch.zeros(num_motions_to_try * num_videos, G.w_dim, device=device))['motion_noise'] # [num_videos * num_motions_to_try, ...]
    ...
    motion_noise_opt = motions[best_motions_idx] # [num_videos, ...]

    return motion_noise_opt

#----------------------------------------------------------------------------

@torch.no_grad()
def find_w_init(G: "Generator", vgg16: nn.Module, target_features: Tensor, c: Tensor, t: Tensor, l: Tensor, num_w_to_try: int=128):
    ...
    synth_images = G.synthesis(
        ...
        t=ts.repeat_interleave(num_w_to_try, dim=0),
        l=l.repeat_interleave(num_w_to_try, dim=0))
    if synth_images.shape[2] > 256:
        synth_images = F.interpolate(synth_images, size=(256, 256), mode='area')
    synth_images = (synth_images * 0.5 + 0.5) * 255.0
    synth_features = vgg16(synth_images, resize_images=False, return_lpips=True) # [num_videos * num_motions_to_try, ...]
...
def extract_faces_from_images(_sentinel=None, imgs: List=None, ref_image: "Image"=None, device: str='cuda'):
    ...
    try:
        ...
        raise ImportError("`face_alignment` library must be installed.")
...
def save_edited_w(
    ...
    with torch.no_grad():
        ...
        l = torch.zeros(num_videos, device=device) # [num_videos]

        for i, w in enumerate(all_w):
            torch.save(w.cpu(), os.path.join(w_outdir, f'{img_names[i]}_w.pt'))

            if all_motion_noise is None:
                motion_noise = None
            else:
                motion_noise = all_motion_noise[i] # [...<any>...]
                torch.save(motion_noise.cpu(), os.path.join(w_outdir, f'{img_names[i]}_motion.pt'))
                motion_noise = motion_noise.unsqueeze(0).to(device) # [1, ...<any>...]
                motion_noise = torch.randn_like(motion_noise)
            ...
            imgs = G.synthesis(w, c=c[[i]], l=l[[i]], t=t, motion_noise=motion_noise)
...
def main(
    ...
    assert not G.cfg.use_video_len_cond, "G.cfg.use_video_len_cond is not supported"

    w_all_iters, motion_noise_final = project(
        G=G,
    ...
    save_edited_w(
        ...
        all_w = w_all_iters[-1],
        all_motion_noise = motion_noise_final,
