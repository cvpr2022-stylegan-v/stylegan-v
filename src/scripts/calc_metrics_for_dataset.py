...
def subprocess_fn(rank, args, temp_dir):
    ...
    for metric in args.metrics:
        ...
        result_dict = metric_main.calc_metric(
            ...
            device=device,
            progress=progress,
            cache=(True if metric == 'fid50k_full' else args.use_cache),
...
def calc_metrics_for_dataset(ctx, metrics, real_data_path, fake_data_path, mirror, resolution, gpus, verbose, use_cache: bool, num_runs: int):
    ...
    if args.verbose:
        print('Generator dataset options:')
...
def calc_metrics_wrapper(ctx, *args, **kwargs):
    calc_metrics_for_dataset(ctx, *args, **kwargs)

#----------------------------------------------------------------------------

if __name__ == "__main__":
    calc_metrics_wrapper() # pylint: disable=no-value-for-parameter
