import sys; sys.path.extend(['.', 'src', '/PATH/TO/StyleCLIP'])
import argparse
import math
import os
...
def run_edit_optimization(
    ...
    ir_se50_weights: str="/PATH/TO/StyleCLIP/model_ir_se50.pth",
    mask: float=None,
    ...
    l = torch.zeros(num_prompts * num_images, device=device)
    ...
    with torch.no_grad():
        img_orig = G.synthesis(ws=w_orig, c=c, l=l, t=ts) # [num_prompts * num_images, 3, c, h, w]
    ...
    for curr_iter in pbar:
        ...
        img_gen = G.synthesis(ws=w, c=c, l=l, t=ts) # [num_prompts * num_images, 3, c, h, w]
